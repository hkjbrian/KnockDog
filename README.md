...
4. **카카오로 로그인**을 누르면 페이지가 refresh JWT 쿠키로 access JWT를 재발급한다. 이후 역할 확정·토큰 재발급·로그아웃을 눌러 응답을 확인한다. `KD_REFRESH`만 HttpOnly 쿠키인지와 요청의 `Authorization: Bearer` 헤더를 확인한다.
